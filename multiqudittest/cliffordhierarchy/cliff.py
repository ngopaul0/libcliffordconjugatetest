...
def UfromCzCx2(CzCx):
  ...
  V1 = CzCx[0][1]
  V2 = CzCx[1][1]
  x = np.array([(matrix_power(V1,a) @ matrix_power(V2,b) @ Ket0).reshape(d**2)  for a in Zd for b in Zd])
  #x  = np.array([(np.dot(matrix_power(V1,a) @ matrix_power(V2,b), Ket0)).reshape(9)  for a in Zd for b in Zd])
...
if __name__ == "__main__":
    ...
    file_name = 'n2-c2-d' + str(d) + '-asAP.npy'
    if os.path.exists(file_name):
      with open(file_name, 'rb') as f:
        C2ap = np.load(f, allow_pickle=True)
      print(f"File '{file_name}' loaded successfully. Loaded {len(C2ap)} conjugate tuples")
    else:
      ...
      # This is larger than the actual gates
      #with open(file_name, 'wb') as f:                     # Save data file of C3 gates
      #  np.save(f, C2ap)
      #print("Written")
    print("Generating Clifford gates")
    C2 = pool_map(UfromCzCx2, C2ap)                            # Generate Cliffords (up to phase)    
    print(f"Created {len(C2)} gates. Writing to file")
    with open('n2-c2-gates-d' + str(d) + '-asGATES.npy', 'wb') as f:                     # Save data file of C3 gates
      np.save(f, C2)
    print("Written gates")
    print(f"First gate is {C2[0]} and has dimension {C2[0].shape} and type {type(C2[0])}")
