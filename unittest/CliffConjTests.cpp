...
TEST_CASE("Equal matrices are Clifford-conjugate", "[equal]") {
    SECTION("Two identical matrices should be equal") {
        Eigen::Matrix2cd M;
        M << std::complex(1.0, 2.0), std::complex(3.0, 4.0),
             std::complex(5.0, 6.0), std::complex(7.0, 8.0);

        Eigen::Matrix2cd M_prime;
        M_prime << std::complex(1.0, 2.0), std::complex(3.0, 4.0),
                   std::complex(5.0, 6.0), std::complex(7.0, 8.0);
...
TEST_CASE("multiple Pauli basis elements", "[multiple]") {
    SECTION("d = 3: Two Pauli basis elements") {
        ...
        expected << std::complex(0.0, 0.0),             std::complex(-0.5, 0.8660254040),     std::complex(-0.5000000002, -0.8660254040),
                    std::complex(-0.5000000002, 0.8660254042), std::complex(0.0, 0.0),             std::complex(1.0, 0.0),
                    std::complex(-0.5000000002, -0.8660254040), std::complex(1.0, 0.0),             std::complex(0.0, 0.0);
        INFO((result) << " vs " << (expected));
        for (size_t i = 0; i < result.rows(); ++i) {
            for (size_t j = 0; j < result.cols(); ++j) {
                REQUIRE_THAT(result(i,j).imag(), Catch::Matchers::WithinAbs(expected(i,j).imag(), 1e-5));
                REQUIRE_THAT(result(i,j).real(), Catch::Matchers::WithinAbs(expected(i,j).real(), 1e-5));
            }
        }

        result = Mprime;
        Eigen::Matrix3cd expectedMprime;
        expectedMprime << std::complex(0.0, 0.0),               std::complex(-0.5000000004, -0.8660254040), std::complex(-0.5000000000, 0.8660254040),
                    std::complex(-0.5000000004, -0.8660254040), std::complex(0.0, 0.0),               std::complex(1.000000001, -0.0000000003233738859),
                    std::complex(-0.5000000002, 0.8660254043),  std::complex(1.000000000, 0.0),       std::complex(0.0, 0.0);
        INFO((result) << " vs " << (expected));
        for (size_t i = 0; i < result.rows(); ++i) {
            for (size_t j = 0; j < result.cols(); ++j) {
                REQUIRE_THAT(result(i,j).imag(), Catch::Matchers::WithinAbs(expectedMprime(i,j).imag(), 1e-5));
                REQUIRE_THAT(result(i,j).real(), Catch::Matchers::WithinAbs(expectedMprime(i,j).real(), 1e-5));
            }
        }
