...
using TupleTestType3 = Eigen::Vector<long, 3>;
using TupleTestType4 = Eigen::Vector<long, 4>;
...
TEST_CASE("TupleIterator", "[tupleiterator]") {
    SECTION("Iterates correctly over all elements of Z_3^4 with UseVariableModuli = false") {
        std::vector<TupleTestType4> allValues;
    ...
    SECTION("Increment and decrement operators") {
        ...
        REQUIRE((*it) == TupleTestType3(0, 0, 0));

        ++it; // Pre-increment
        REQUIRE((*it) == TupleTestType3(0, 0, 1));

        auto temp_it = it++; // Post-increment
        REQUIRE((*temp_it) == TupleTestType3(0, 0, 1));
        REQUIRE((*it) == TupleTestType3(0, 0, 2));
    ...
    SECTION("Random access operations") {
        ...
        REQUIRE((*it_plus_5) == TupleTestType3(0, 1, 1));

        // Operator +=
        auto it_compound = it_begin;
        it_compound += 8;
        REQUIRE((*it_compound) == TupleTestType3(0, 2, 0));

        // Operator -
        auto it_minus_2 = it_compound - 2;
        REQUIRE((*it_minus_2) == TupleTestType3(0, 1, 2));
        ...
        REQUIRE((*it_compound) == TupleTestType3(0, 1, 0));
